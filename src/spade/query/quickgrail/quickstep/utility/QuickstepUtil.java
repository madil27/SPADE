/*
 ...
 --------------------------------------------------------------------------------
 */
package spade.query.quickgrail.quickstep.utility;
...
public class QuickstepUtil
{
	...
	public static void CreateEmptyGraph(QuickstepExecutor qs, QuickstepGraph graph)
	{
	...
	public static void CreateEmptyGraphMetadata(QuickstepExecutor qs, QuickstepGraphMetadata metadata)
	{
	...
	public static long GetNumVertices(QuickstepExecutor qs, QuickstepGraph graph)
	{
		return qs.executeQueryForLongResult(
				"COPY SELECT COUNT(*) FROM " + graph.getVertexTableName() + " TO stdout;");
	}

	public static long GetNumEdges(QuickstepExecutor qs, QuickstepGraph graph)
	{
		return qs.executeQueryForLongResult(
				"COPY SELECT COUNT(*) FROM " + graph.getEdgeTableName() + " TO stdout;");
	}

	public static long GetNumTimestamps(QuickstepExecutor qs, QuickstepGraph graph)
	{
		return qs.executeQueryForLongResult(
	...
	public static Long[] GetTimestampRange(QuickstepExecutor qs, QuickstepGraph graph)
	{
	...
	public static String[] GetTimestampRangeString(QuickstepExecutor qs, QuickstepGraph graph)
	{
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("E, dd MMM yyyy HH:mm:ss z");
		Long[] span = spade.query.quickgrail.quickstep.utility.QuickstepUtil.GetTimestampRange(qs, graph);
		String startDateStr = "";
		String endDateStr = "";
		if(span[0] != 0)
		{
